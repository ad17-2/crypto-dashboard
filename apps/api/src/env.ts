...
export interface AppEnv {
  configPath: string;
  /** Loaded config with `storage_path` already overridden by CRYPTO_SCREENER_DB_PATH, if set. */
  config: AppConfig;
  dbPath: string;
  reportDir: string;
  apiPort: number;
  ...
  retainRuns: number;
  /**
   * Refresh token gating POST /api/refresh. `null` when CRYPTO_DASHBOARD_REFRESH_TOKEN is unset —
   * callers MUST treat `null` as "always deny" (403), never as "allow any request". See
   * `isRefreshAllowed`.
   */
...
/** Reads and strictly validates the subset of `process.env` this app depends on. */
 ...
 * mismatched byte, leaking the correct token's length/prefix through response-time variance.
 * `timingSafeEqual` requires equal-length buffers, so a length mismatch is handled separately
 * (and still runs a same-cost dummy comparison, so a wrong-length guess isn't distinguishable by
 * timing from a same-length wrong guess).
 */
